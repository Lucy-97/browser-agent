...
- 2026-07-20：完成 artifact storage backend 抽象；本地继续使用 `ARTIFACT_DIR`，生产切换到私有 Cloudflare R2，并通过租户鉴权 API 受控下载。
...
5. 已完成 `local`/`r2` storage backend 抽象；后续补 R2 生命周期删除、容量配额、内容扫描、故障告警和 staging 实桶 E2E。
